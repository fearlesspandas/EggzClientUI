...
func update_mesh_from_position(location:Vector3):
	if (self.global_transform.origin - location).length_squared() > ClientSettings.CAMERA_RENDER_DISTANCE*ClientSettings.CAMERA_RENDER_DISTANCE :
		#TerrainSignalsClient.add_to_navigation_mesh(uuid,self.global_transform.origin, Color.red,3)
		loaded = false
	else:

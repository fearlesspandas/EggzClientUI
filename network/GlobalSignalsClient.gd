...
#INVENTORY SIGNALS
signal pocket(client_id,contents)
func pocket(client_id,contents):
	emit_signal("pocket",client_id,contents)
...
#emits unpocketed_item signal
signal unpocketed_item(client_id,item,amount)
...
func field(client_id,contents,occupied):
	emit_signal("field",client_id,contents,occupied)
